...
| **Output Quality** | Can produce non-words or incorrect roots (e.g., "policies" → "polici"). | Always produces a valid dictionary word (e.g., "policies" → "policy"). |
...
| **Suitable Applications** | Search engines, information retrieval, large-scale indexing of complaint archives where minor errors are acceptable. | Grievance chatbots, translation of bilingual complaints, sentiment analysis of customer feedback — tasks where correct word meaning directly affects output quality. |

## Example
**Word:** "policies"
- Stemming output: `polici` (not a real word)
- Lemmatization output: `policy` (correct dictionary form)

**Word:** "paid"
- Stemming output: `paid` (unchanged, since stemmers work on suffixes only and "paid" has no suffix to strip)
- Lemmatization output: `pay` (correctly identifies "paid" as the past tense of "pay")

## Why this matters for morphologically rich languages
Languages like Hindi and Marathi have far more complex word inflections than English (multiple suffixes can attach to a single root depending on gender, tense, and case). This makes naive suffix-stripping stemmers particularly unreliable for these languages, since a single wrong rule can distort many different words. Lemmatization, which relies on structured linguistic knowledge, is generally the safer choice when working with such languages — which matters directly for a bilingual banking grievance system expected to handle Hindi/Marathi complaints alongside English ones — though it also requires much richer dictionaries than are readily available for many Indian languages.

## Conclusion
Stemming is preferred when speed matters more than precision (e.g., indexing millions of archived complaint records). Lemmatization is preferred when the meaning of the word must be preserved accurately, especially in tasks like grievance translation or a conversational banking chatbot.
