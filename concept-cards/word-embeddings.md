...
## 2. Purpose
Machine learning models cannot work directly with text — they need numbers. Earlier representation methods (like one-hot encoding) treated every word as completely unrelated to every other word, so a grievance-classification model would see "rejected" and "denied" as two totally different symbols. Word embeddings solve this by encoding meaning: words used in similar contexts end up with similar vector representations.
...
- Once trained, vector arithmetic captures relationships — for example, the vector for "approved" minus "loan" plus "card" lands close to the vector for "card approved."
- Popular embedding models include Word2Vec, GloVe, and FastText.

## 4. Example
In a well-trained embedding space, trained on banking complaint text:
- "rejected" and "denied" would have vectors close to each other (since "loan rejected" and "loan denied" mean the same thing to a customer).
- "rejected" and "weather" would have vectors far apart (unrelated meaning).
...
## 6. Limitations
- A single word gets only one fixed vector, so it cannot capture different meanings in different contexts (e.g., "bank" as a financial institution vs. a riverbank — a genuinely confusing case for a banking grievance system if the corpus is noisy).
- Requires large amounts of training text to produce good-quality embeddings.
- Struggles with rare words or words not seen during training (out-of-vocabulary problem) — a real issue for informal complaint text full of typos and abbreviations like "txn" for "transaction."
